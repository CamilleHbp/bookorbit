...
describe('Fanfiction settings API contract', () => {
  ...
  it('creates a Fiction.live preset without age confirmation and selects the saved profile for testing', async () => {
    const state = settings()
    state.libraryId.value = 5
    state.newProfile()
  ...
  it('reopens the correct site section and preserves secrets when saving a profile', async () => {
    const state = settings()
    state.libraryId.value = 5
    const profile = { id: 'profile', libraryId: 5, name: 'My fiction', version: 3, updatedAt: '' }
    mockApi.mockResolvedValueOnce(response({ ...profile, configuration: '[fiction.live]\nusername = reader\npassword = ********\n', cookies: [] }))
    await state.editProfile(profile)
    expect(state.section.value).toBe('fiction.live')
  ...
  it('does not carry credentials from one site preset to another', () => {
    const state = settings()
    state.newProfile()

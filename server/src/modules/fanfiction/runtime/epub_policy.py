...
class EpubPolicyError(PolicyError):
    code = 'invalid_epub'
...
def validate_epub(path):
    with ZipFile(path) as archive:
        ...
        if not entries or len(entries) > 10_000:
            raise EpubPolicyError('Archive entry limit exceeded')
        names = set()
        expanded = 0

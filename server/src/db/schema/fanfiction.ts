...
export const fanfictionSources = pgTable(
  'fanfiction_sources',
  {
    ...
    categories: jsonb('categories').$type<FanfictionCategories>(),
    tagPolicy: varchar('tag_policy', { length: 20 }).$type<'review' | 'automatic'>().notNull().default('review'),

...
- Remotes: `upstream` is `https://github.com/bookorbit/bookorbit.git`; `origin` is `git@github.com:CamilleHbp/bookorbit.git`. Fetch both before work. Push only to `origin` unless explicitly instructed otherwise.
- Start every independent feature from the current `upstream/main`, never from the fork's integration `main` or another feature. Long-lived feature branches are `codex/fanfiction` and `codex/nsfw-covers`.
- Optional subwork branches start from their owning feature branch and merge back into that feature branch after verification. Use `codex/fanfiction-<description>` or `codex/nsfw-covers-<description>`; issue-linked branches use `BO-<issue-number>-<description>` with the same feature base.
- Keep the two features independent. Update their upstream baseline directly from `upstream/main`. Never merge `origin/main` or an unrelated feature into a feature branch.
- `origin/main` is the fork's integration and deployment branch. After completing a verified increment, merge it into its feature branch, merge the feature into `main`, push both to `origin`, and deploy the exact `origin/main` commit through HomeServer. Development may continue on either feature after deployment.
- Reconcile conflicting generated migrations during integration and validate fresh and existing-database upgrade paths in isolated databases. Preserve applied migration SQL and hashes; do not assume branch migration numbers match the live ledger.
- Only the user decides when a feature is finished and ready for an upstream PR. Do not open, update, or merge upstream PRs without that explicit instruction. Prepare each PR against `upstream/main` from its independent feature history, excluding fork-only configuration commits.
- Keep fork workflow instructions and Serena configuration in dedicated commits, separate from feature implementation. Keep these instructions available on feature and subwork branches too.
- Preserve unrelated local changes. Archive superseded branch tips before deleting branches or rewriting history. Never overwrite newly arrived remote work; use an explicit expected SHA with `--force-with-lease` when an authorized rewrite requires it.
- Follow [commit guidelines](docs/COMMIT_GUIDELINES.md). Never add `Co-authored-by` trailers.
...
- Deployment lives at `/home/debian/HomeServer` on that host; read its `AGENTS.md` before deployment operations. Local counterpart: `/Users/camille/Dev/Personal/HomeServer`. Deploy verified increments from `origin/main`; do not deploy unverified or broken work.
